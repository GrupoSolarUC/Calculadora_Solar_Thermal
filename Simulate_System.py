...
def raise_error(error_message):
    '''
    Función para mostrar una ventana de error.

    Parámetros:
        - error_message: mensaje a mostrar en la ventana
...
if demand_type == 1:
    ...
    def demand_func(t):
        '''
        Función que entrega el flujo demandado en función de un instante del año

        Parámetros:
            - t: Instante para el cual se quiere conocer la demanda, en horas (por lo tanto, su valor está en el rango [0,8760]).
...
try:
    ...
    raise_error("No se encuentra el archivo de datos meteorológicos. Este archivo debe estar en la misma carpeta donde se encuentra el Script 'Simulate_System.py'")
    params_file.close()
    sys.exit()
...
DHI = np.array([ 3.6*float(climate_data[i][7]) for i in [8801] + list(range(42, 8801)) ])
    ...
    if operation:
        ...
        while True:
            Field_outputs = Field.compute_outputs(solar_field_flow, previous_enthalpies['h_in_solar_field'],
                                                  h_mains_field, beam, diff, diff_ground,
...
## Begin simulation from January 1 to December 31. (Now the results are recorded)
...
## Compute daily consumption in liters and generate a plot of the demand curve.
...
## Create Maps of the temperature of the flow leaving the tank, the solar fraction and the heating power of the solar field
...
if Q_waste_counter > 0:
    Heat_Map(Q_waste, 'Calor purgado en forma de vapor [W]', 'Q_waste')
    index_max_Q_waste = np.argmax(Q_waste)
    max_Q_waste = np.round(Q_waste[index_max_Q_waste],2)
    max_Q_waste_percentage = np.round(100*Q_waste[index_max_Q_waste]/Q_useful[index_max_Q_waste],2)
...
## Create text file to store the simulation parameters

Parameters_file = open('Parametros_Simulacion.txt','w')
Parameters_file.write('Número de la simulación: '+str(simulation_number)+'\n')
Parameters_file.write('Nombre de archivo con datos meteorológicos: '+climate_data_file_name+'\n')
...
## Create file with several relevant results and aclarations
...
Results_file.write('El calor total recibido por el tanque de almacenamiento (esto es equivalente al calor total producido por el campo solar menos el calor "purgado" en forma de vapor en el campo solar y en el intercambiador de calor) durante el año fue de '+str(np.round(sum(Q_useful)*time_step/1000,2))+' kWh.\n')
Results_file.write('La potencia máxima de calor recibida por el tanque (o equivalentemente, calor no purgado del campo solar y del intercambiador de calor) fue de '+str(np.round(max(Q_useful),2))+' W.\n')
Results_file.write('El horario en los mapas está ajustado al horario UTC-3 (horario de verano en Chile). Es normal que se observe un desplazamiento en la demanda en invierno debido a los cambios de hora. Se asume que éstos se realizan los días 7 de abril y 8 de septiembre a las 24:00 horas.\n')
Results_file.write("El mapa 'Calor útil aportado por campo solar' corresponde al calor útil (no purgado en forma de vapor) que pasa a través del intercambiador de calor. Dicha potencia será igual a la potencia útil (no purgada) producida por el campo solar, a menos que se esté generando vapor en el intercambiador de calor.\n")
...
Results_file.write('Fracción solar anual: '+str(np.round(100*Total_Solar_Fraction,2))+'%\n')
if yearly_cost != None:
...
Results_file.write('Fracción solar mínima: '+str(np.round(100*minimum_solar_frac,2))+'%\n')
Results_file.write('Fracción solar máxima: '+str(np.round(100*maximum_solar_frac,2))+'%\n')
if excess_temp_counter > 0:
    ...
    Results_file.write('Considere reducir el tamaño del campo solar si la temperatura alcanzada fuese excesiva, o si el porcentaje del tiempo durante el cual se excedió la temperatura deseada fuese considerable.\n')
else:
...
if Q_waste_counter > 0:
    ...
    Results_file.write("La potencia 'perdida' en forma de vapor alcanzó un valor máximo de "+str(max_Q_waste)+'W; esto representa un '+str(max_Q_waste_percentage)+'% del calor útil que se estaba produciendo en ese momento.\n')
Results_file.close()

## Create file with the detailed results of each simulation time step
...
new_T_in_solar_field = []
new_T_out_solar_field = []
new_T_in_HX_load = []
new_T_out_HX_load = []
for i in range(len(Result['operation'])):
    if Result['operation'][i]:
        new_T_in_solar_field.append(Result['T_in_solar_field'][i])
        new_T_out_solar_field.append(Result['T_out_solar_field'][i])
        new_T_in_HX_load.append(Result['T_in_HX_cold'][i])
        new_T_out_HX_load.append(Result['T_out_HX_cold'][i])
    else:
        new_T_in_solar_field.append('None')
        new_T_out_solar_field.append('None')
        new_T_in_HX_load.append('None')
        new_T_out_HX_load.append('None')
Result2['T_in_solar_field'] = new_T_in_solar_field
Result2['T_out_solar_field'] = new_T_out_solar_field
Result2['T_in_HX_load_side'] = new_T_in_HX_load
Result2['T_out_HX_load_side'] = new_T_out_HX_load
